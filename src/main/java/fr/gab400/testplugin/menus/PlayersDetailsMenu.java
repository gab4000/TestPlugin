...
public class PlayersDetailsMenu extends Menu {
	
	private final Player target;
	
	public PlayersDetailsMenu(Player owner, Player target) {
		super(owner);
		this.target = target;
	}
	...
	public @NotNull Map<Integer, ItemStack> getContent() {
		...
		map.put(7, new ItemBuilder(this, Material.PACKED_ICE, itemMeta -> {
			...
			itemMeta.setLore(List.of(ChatColor.BLUE + "État : " + (Core.frozen_players.contains(target) ? "§4Freeze" : "§2Unfreeze")));
		}).setOnClick(inventoryClickEvent -> {
			if (!checkOnline()) return;
